...
class Config {
	...
	protected function __construct($args = NULL) {
		...
		$this->logger = Logger::getInstance($this->{'error-log-level'}, $this->{'error-log'}, '.', Logger::NYSS_LOG_MODE_STDOUT);

		// TODO: need a PSR-4 DAL package.  Until then, this is da way.
    // Register a default database with Drupal's DAL.
		global $databases;
	...
	public function populateCommands($object = NULL) {
		...
		$cmd      = Command\CommandBase::createFromTemplate('help', $help_cmd);
		$this->getopt->addCommand($cmd);
		if (is_object($object)) {
			foreach ($object as $key => $val) {
				$cmd = Command\CommandBase::createFromTemplate($key, $val);
				$this->getopt->addCommand($cmd);
			}
		}
	}
  ...
  /**
   * Populates the config object with the INI values.
   *
   * @param $cfg_file
   */
  ...
  /**
   * Retrieve a GetOpt\Command object by name.
   *
   * @param null $name
   *
  ...
  /**
   * Exposes the help text from the underlying GetOpt object.
   *
   * @return string
   */
